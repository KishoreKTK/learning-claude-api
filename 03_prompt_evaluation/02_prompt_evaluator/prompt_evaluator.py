...
def chat(messages, system=None, max_tokens=1000, stop_sequences=None):
    """
    Small compatibility wrapper for Anthropic SDK versions that do not accept
    `temperature` in messages.create().
    """
    params = {
        "model": MODEL_NAME,
...
class PromptEvaluator:
    ...
    def generate_dataset(
        ...
        """Generate a test dataset and save it to file."""
        if prompt_inputs_spec is None:
            prompt_inputs_spec = {}
...
def demo_run_prompt(prompt_inputs):
    prompt = f"""
...
def main():
    ...
    print(f"Generated {len(dataset)} dataset items at {dataset_path}")

    results = evaluator.run_evaluation(
        run_prompt_function=demo_run_prompt,
        ...
        """,
        json_output_file=str(Path(__file__).resolve().parent / "output.json"),
        html_output_file=str(Path(__file__).resolve().parent / "output.html"),
    )

    print("Evaluation complete.")
    print(results[0]["score"] if results else "No results")
